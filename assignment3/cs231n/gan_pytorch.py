...
def sample_noise(batch_size, dim, seed=None):
    ...
    sample = 2 * torch.rand(batch_size, dim) - 1
    return sample
...
def discriminator_loss(logits_real, logits_fake):
    ...
    N = logits_real.shape
    targets_real = torch.ones(N).type(dtype)
    targets_fake = torch.zeros(N).type(dtype)
    loss_real = bce_loss(logits_real, targets_real)
    loss_fake = bce_loss(logits_fake, targets_fake)
...
def get_optimizer(model):
    ...
    optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), betas=(0.5, 0.999))
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    N = scores_real.shape
    targets_real = torch.ones(N).type(dtype)
    loss = 0.5*(torch.mean((scores_real-targets_real)**2) + torch.mean(scores_fake**2))
...
def ls_generator_loss(scores_fake):
    ...
    N = scores_fake.shape
    ones = torch.ones(N).type(dtype)
    loss = 0.5*(torch.mean((scores_fake-ones)**2))
...
def build_dc_generator(noise_dim=NOISE_DIM):
    ...
    class Model(nn.Module):
      ...
      def forward(self, x):
          ...
          x = self.bn3(self.relu(self.deconv1(x)))
