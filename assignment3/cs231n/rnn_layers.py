...
def rnn_step_backward(dnext_h, cache):
    ...
    Wh, next_h, Wx, prev_h, x = cache
    N, H = dnext_h.shape
    dprev_h = np.zeros((N,H))
    dx = np.zeros((N,Wx.shape[0]))
    dtgh = 1-next_h**2
    temp = dnext_h * dtgh
    ...
    for n in range(N):
      dnext_h_to_dprev_h = Wh.dot(np.diag(dtgh[n,:]))
      dnext_h_to_dx = Wx.dot(np.diag(dtgh[n,:]))
      dprev_h[n,:] = dnext_h[n,:].dot(dnext_h_to_dprev_h.T)
      dx[n,:] = dnext_h[n,:].dot(dnext_h_to_dx.T)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
      curr_x = x[:,t,:]
      prev_h = h[-1]
      curr_h, curr_cache = rnn_step_forward(curr_x, prev_h, Wx, Wh, b)
      h.append(curr_h)
      cache.append(curr_cache)
    h = np.array(h[1:])
    h = np.moveaxis(h, 0,1) #first h is no needed for backpropegation
...
def rnn_backward(dh, cache):
    ...
    for t in range(-1, -T, -1):
      dcurr_x, dprev_h, dcurr_Wx, dcurr_Wh, dcurr_b = rnn_step_backward(curr_dh, cache[t])
      curr_dh = dh[:,t-1,:] + dprev_h
      dx.insert(0, dcurr_x)
      dWx += dcurr_Wx
      dWh += dcurr_Wh
      db += dcurr_b
...
def word_embedding_backward(dout, cache):
    ...
    for n in range(N):
      samp_x = x[n,...]
      samp_dout = dout[n,...]
      one_hot_x = np.eye(V)[samp_x]
      dW += one_hot_x.T.dot(samp_dout)
